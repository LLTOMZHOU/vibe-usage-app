...
if rg -n '@vibe-cafe/vibe-usage@latest|\["--yes"|\["x", packageSpecifier' \
...
if rg -n 'parseCursor|parseAntigravity' VibeUsage/Resources/vibe-usage-cli/src/parsers/index.js; then
    fail "a prohibited parser is registered"
fi

if rg -n 'SUFeedURL|SUPublicEDKey|SUEnableAutomaticChecks' VibeUsage/Info.plist; then
    fail "automatic remote update configuration returned"
fi

rg -q '6c72607286b125488003c741c280d4cce6263d1f' \
