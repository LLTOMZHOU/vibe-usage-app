...
export async function runSync({
  ...
  const apiUrl = process.env.VIBE_USAGE_SURFACE === 'mac-app'
    ? 'https://vibecafe.ai'
    : configuredApiUrl;
  if (process.env.VIBE_USAGE_SURFACE === 'mac-app') {
    let approved = false;
    try {
      const parsed = new URL(apiUrl);
      approved = parsed.protocol === 'https:'
        && parsed.hostname === 'vibecafe.ai'
        && !parsed.username
        && !parsed.password
        && !parsed.port;
    } catch {
  ...
  let allBuckets = [];
  let allSessions = [];
  const uploadSessions = process.env.VIBE_USAGE_UPLOAD_SESSIONS?.trim() === '1';
  const parserResults = [];
  ...
  for (const { source, result, error } of parserOutcomes) {
    ...
    if (uploadSessions && sessions.length > 0) allSessions.push(...sessions);
    if (buckets.length > 0 || uploadSessions && sessions.length > 0) {
      parserResults.push({ source, buckets: buckets.length, sessions: uploadSessions ? sessions.length : 0 });
    }
  }
  ...
  let host = process.env.VIBE_USAGE_HOSTNAME?.trim() || config.hostname;
  if (!host) {
    host = 'Mac-private';
    config.hostname = host;
    saveConfig(config);
  }
  // Parsers may pre-set their own hostname sentinel. Otherwise every record
  // receives the app's random device alias.
  ...
  if (!uploadSessions) {
    if (!quiet && allSessions.length > 0) {
      console.log(dim('  会话统计: 已在本机隐私设置中关闭'));
    }
    allSessions = [];
  }
