...
1. **Local value comes first.** Prefer features that work from data already on
   the user's Mac. Cloud features must earn the data they require.
...
Security and privacy are phase one. Natural additions include richer local
analysis, clearer data previews before upload, user-owned exports and backups,
selective or self-hosted sync, and original insights that do not require a
public identity. Social or comparative features can exist, but only as explicit
opt-ins with a clear account-level state.
